        ...
        <el-col :span="16">
          ...
          >
            ...
            <el-form-item label="班级" prop="class">
              <el-input v-model="profileForm.class" disabled />
            </el-form-item>
...
import { sendEmailVerificationCode } from '@/utils/emailCode'
...
const initUserInfo = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      // 填充表单数据
      profileForm.userId = user.value?.userId || '';
      profileForm.name = user.value?.name || '';
      profileForm.gender = user.value?.gender || '';
      profileForm.class = user.value?.classId || '';
      ...
      userAvatar.value = user.value?.avatarUrl
        ? `/src/assets/avatar/student/${user.value?.avatarUrl}`
  ...
  await profileFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        // 调用工具函数更新学生信息
        const result = await updateStudentProfile(studentInfo);
